...
std::filesystem::path
canonicalize_existing_path(const std::filesystem::path &path,
  ...
  if (absolute_error) {
    throw std::runtime_error("Failed to build absolute path for " + label +
  ...
  if (canonical_error) {
    throw std::runtime_error("Failed to canonicalize " + label + ": " +
...
std::filesystem::path
normalize_absolute_path(const std::filesystem::path &path) {
  std::error_code absolute_error;
  const std::filesystem::path absolute_path =
      std::filesystem::absolute(path, absolute_error);

  if (absolute_error) {
    throw std::runtime_error("Failed to build absolute path: " +
                             absolute_error.message());
  }
...
std::string read_text_file_first_line(const std::filesystem::path &path) {
  ...
  while (non_space_index < line.size() &&
         (line[non_space_index] == ' ' || line[non_space_index] == '\t')) {
    ++non_space_index;
  }
...
std::string make_uid_from_seed(const std::string &seed_prefix,
...
std::string
make_path_derived_uid(const std::filesystem::path &canonical_root,
...
std::string make_generated_uid(const std::filesystem::path &canonical_root,
                               const std::filesystem::path &project_file_path) {
  const std::string seed =
      to_utf8_from_u8(canonical_root.generic_u8string()) + "|" +
      to_utf8_from_u8(project_file_path.generic_u8string()) + "|" +
...
String build_godot_version_string() {
  ...
  String version_text = String::num_int64(major) + "." +
...
std::vector<std::string> normalized_dirty_paths_from_array(const Array &paths) {
  ...
  for (int64_t i = 0; i < paths.size(); ++i) {
    ...
    const std::string path =
        gotool::project_scanner::normalize_project_path(to_utf8(raw));
    if (!path.empty()) {
...
void GodotProjectContext::_bind_methods() {
  ClassDB::bind_method(D_METHOD("initialize_database"),
                       &GodotProjectContext::initialize_database);

  ClassDB::bind_method(D_METHOD("register_current_project"),
                       &GodotProjectContext::register_current_project);

  ClassDB::bind_method(D_METHOD("get_database_virtual_path"),
                       &GodotProjectContext::get_database_virtual_path);

  ClassDB::bind_method(D_METHOD("get_database_absolute_path"),
                       &GodotProjectContext::get_database_absolute_path);

  ClassDB::bind_method(D_METHOD("scan_current_project"),
                       &GodotProjectContext::scan_current_project);

  ClassDB::bind_method(D_METHOD("scan_project"),
                       &GodotProjectContext::scan_project);

  ClassDB::bind_method(D_METHOD("start_scan", "options"),
                       &GodotProjectContext::start_scan);

  ClassDB::bind_method(D_METHOD("cancel_scan", "scan_id"),
                       &GodotProjectContext::cancel_scan);

  ClassDB::bind_method(D_METHOD("get_scan_status", "scan_id"),
                       &GodotProjectContext::get_scan_status);

  ClassDB::bind_method(D_METHOD("get_scan_metrics", "scan_id"),
                       &GodotProjectContext::get_scan_metrics);

  ClassDB::bind_method(D_METHOD("scan_project_inventory_fast", "options"),
                       &GodotProjectContext::scan_project_inventory_fast);

  ClassDB::bind_method(D_METHOD("scan_current_project_fast", "options"),
                       &GodotProjectContext::scan_current_project_fast);

  ClassDB::bind_method(D_METHOD("benchmark_native_scan", "options"),
                       &GodotProjectContext::benchmark_native_scan);

  ClassDB::bind_method(D_METHOD("start_watcher"),
                       &GodotProjectContext::start_watcher);

  ClassDB::bind_method(D_METHOD("stop_watcher"),
                       &GodotProjectContext::stop_watcher);

  ClassDB::bind_method(D_METHOD("get_watcher_status"),
                       &GodotProjectContext::get_watcher_status);

  ClassDB::bind_method(D_METHOD("consume_watcher_changes"),
                       &GodotProjectContext::consume_watcher_changes);

  ClassDB::bind_method(D_METHOD("get_dirty_paths"),
                       &GodotProjectContext::get_dirty_paths);

  ClassDB::bind_method(D_METHOD("get_file_count", "filter"),
  ...
  ClassDB::bind_method(D_METHOD("get_file_details", "file_id"),
                       &GodotProjectContext::get_file_details);

  ClassDB::bind_method(D_METHOD("get_directory_children", "directory_id",
                                "offset", "limit", "sort", "filter"),
                       &GodotProjectContext::get_directory_children);

  ClassDB::bind_method(D_METHOD("get_custom_class_count", "filter"),
  ...
  ClassDB::bind_method(D_METHOD("list_dependents_of_file", "target_file_id"),
                       &GodotProjectContext::list_dependents_of_file);

  ClassDB::bind_method(D_METHOD("list_dependents_of_class", "class_name"),
                       &GodotProjectContext::list_dependents_of_class);

  ClassDB::bind_method(D_METHOD("list_unresolved_dependencies"),
                       &GodotProjectContext::list_unresolved_dependencies);

  ClassDB::bind_method(D_METHOD("list_dynamic_dependencies"),
  ...
  ClassDB::bind_method(D_METHOD("list_functions_for_script", "script_file_id"),
                       &GodotProjectContext::list_functions_for_script);

  ClassDB::bind_method(D_METHOD("list_properties_for_script", "script_file_id"),
  ...
  ClassDB::bind_method(D_METHOD("list_doc_comment_gaps", "filter"),
                       &GodotProjectContext::list_doc_comment_gaps);

  ClassDB::bind_method(D_METHOD("list_references_for_script", "script_file_id"),
                       &GodotProjectContext::list_references_for_script);

  ClassDB::bind_method(D_METHOD("list_references_from_symbol", "symbol_id"),
                       &GodotProjectContext::list_references_from_symbol);

  ClassDB::bind_method(D_METHOD("list_unresolved_references", "filter"),
                       &GodotProjectContext::list_unresolved_references);

  ClassDB::bind_method(D_METHOD("list_dynamic_references", "filter"),
                       &GodotProjectContext::list_dynamic_references);

  ClassDB::bind_method(D_METHOD("list_scene_script_attachments", "filter"),
                       &GodotProjectContext::list_scene_script_attachments);

  ClassDB::bind_method(D_METHOD("list_scenes_using_script", "script_file_id"),
  ...
  ClassDB::bind_method(D_METHOD("get_symbol_details", "symbol_id"),
  ...
  ClassDB::bind_method(D_METHOD("list_dependency_cycles"),
  ...
  ClassDB::bind_method(D_METHOD("export_full_inventory_for_debug"),
                       &GodotProjectContext::export_full_inventory_for_debug);

  ClassDB::bind_method(D_METHOD("list_projects"),
                       &GodotProjectContext::list_projects);

  ClassDB::bind_method(D_METHOD("get_project_summary", "project_id"),
                       &GodotProjectContext::get_project_summary);

  ClassDB::bind_method(D_METHOD("get_last_scan_results"),
...
std::filesystem::path
GodotProjectContext::get_current_project_root_path() const {
...
void GodotProjectContext::join_finished_scan_worker_if_idle() {
...

  {
    ...
    const bool is_running =
        has_active && (active_scan_state_->status == "queued" ||
...
void GodotProjectContext::stop_scan_worker() {
...

  {
    ...
    if (active_scan_state_ != nullptr &&
        (active_scan_state_->status == "queued" ||
         active_scan_state_->status == "running")) {
      active_scan_state_->cancellation_requested.store(
...
Dictionary GodotProjectContext::active_state_to_summary_dictionary(
    const ActiveScanState &state) const {
...
void GodotProjectContext::sync_last_scan_results_from_active_state() const {
  ...
  if (active_scan_state_->status == "queued" ||
      active_scan_state_->status == "running") {
    return;
  }

  last_scan_results_ = active_state_to_summary_dictionary(*active_scan_state_);
  last_scan_results_["metrics"] =
      metrics_to_dictionary(active_scan_state_->metrics);
...
bool GodotProjectContext::initialize_database() {
  ...
  try {
    std::filesystem::path os_data_root =
        godot_string_to_path(os->get_data_dir());
    ...
    const std::filesystem::path database_path =
        storage_root / "gotool_center.sqlite3";
    {
      std::lock_guard<std::mutex> db_lock(database_mutex_);
      database_ = std::make_unique<gotool::database::Database>(
          to_utf8_from_u8(database_path.generic_u8string()));

      // First pass creates/ensures the v2 schema and projects table. Legacy
      // migration is deferred until register_current_project() supplies a
      // concrete project_id.
    ...
    database_absolute_path_ =
        path_to_godot_string(database_path.lexically_normal());
...
int64_t GodotProjectContext::register_current_project() {
  ...
  try {
    const std::filesystem::path project_root_absolute = normalize_absolute_path(
        godot_string_to_path(project_settings->globalize_path("res://")));

    const std::filesystem::path project_file_absolute =
        normalize_absolute_path(godot_string_to_path(
            project_settings->globalize_path("res://project.godot")));
    ...
    if (project_uid.empty()) {
      std::error_code mkdir_error;
      std::filesystem::create_directories(uid_file_path.parent_path(),
                                          mkdir_error);
      ...
      if (!mkdir_error) {
        ...
        std::ofstream uid_output(uid_file_path,
    ...
    if (project_uid.empty()) {
      ...
      if (identity_warning.empty()) {
        identity_warning = "Falling back to path-derived identity because "
                           "project UID was unavailable.";
      }
      project_uid =
          make_path_derived_uid(project_root_canonical, project_file_canonical);
    }

    String display_name =
        project_settings->get_setting("application/config/name", "");
    ...
    registration_input.observed_at_unix =
...
bool GodotProjectContext::scan_current_project() {
  ...
  const bool completed = active_scan_state_ != nullptr &&
                         active_scan_state_->scan_id == scan_id &&
...
bool GodotProjectContext::scan_project() { return scan_current_project(); }
...
Dictionary GodotProjectContext::start_scan(const Dictionary &options) {
  ...
  try {
    const bool include_hidden =
        static_cast<bool>(options.get("include_hidden", true));
    const bool force_rescan =
        static_cast<bool>(options.get("force_rescan", false));
    const bool include_custom_classes =
        static_cast<bool>(options.get("include_custom_classes", true));
    ...
    const bool include_deleted =
        static_cast<bool>(options.get("include_deleted", false));
    const bool load_existing_snapshot =
        static_cast<bool>(options.get("load_existing_snapshot", true));
    const bool enable_parallel_traversal =
        static_cast<bool>(options.get("enable_parallel_traversal", false));
    const int64_t max_parallel_workers =
        static_cast<int64_t>(options.get("max_parallel_workers", 0));
    ...
    if (options.has("dirty_paths")) {
      dirty_paths = normalized_dirty_paths_from_array(
          Array(options.get("dirty_paths", Array())));
    }
    ...
    if (prefer_watcher_dirty_paths) {
      ...
      if (file_watcher_ != nullptr) {
        ...
        if (!watcher_requires_full_rescan && watcher_status.running &&
            watcher_status.supported && dirty_paths.empty()) {
          const std::vector<gotool::project_scanner::FileWatcherEvent> events =
              file_watcher_->drain_events();
          dirty_paths.reserve(events.size());
          for (const gotool::project_scanner::FileWatcherEvent &event :
               events) {
            const std::string path =
                gotool::project_scanner::normalize_project_path(
                    event.project_relative_path);
            if (!path.empty()) {
          ...
          dirty_paths.erase(std::unique(dirty_paths.begin(), dirty_paths.end()),
                            dirty_paths.end());
        }
      }
    }

    const bool use_dirty_path_filter = prefer_watcher_dirty_paths &&
                                       !watcher_requires_full_rescan &&
                                       !dirty_paths.empty() && !force_rescan;
...

    {
      ...
      if (active_scan_state_ != nullptr &&
        ...
        already_running["scan_generation"] =
            active_scan_state_->scan_generation;
        already_running["status"] = "already_running";
        already_running["active_status"] =
            from_utf8(active_scan_state_->status);
        already_running["started_at_unix"] =
            active_scan_state_->started_at_unix;
...

    {
      ...
      scan_id = repository.create_scan_run(project_id, scan_generation,
                                           started_at_unix);
    }
    ...
    try {
      scan_worker_ = std::thread([this, state, project_root, include_hidden,
                                  force_rescan, include_custom_classes,
                                  include_script_dependencies, include_deleted,
                                  load_existing_snapshot,
                                  enable_parallel_traversal,
                                  max_parallel_workers, use_dirty_path_filter,
                                  dirty_paths]() {
        {
          std::lock_guard<std::mutex> lock(scan_mutex_);
        ...
        try {
          ...
          scan_options.collect_script_dependencies =
          ...
          gotool::project_scanner::NativeScanResult native_result;
          {
            ...
            if (database_ == nullptr) {
              throw std::runtime_error(
                  "Database was released while scan worker was running.");
            }
...

      {
        ...
        if (database_ != nullptr) {
          gotool::project_scanner::ScanRepository repository(*database_);
          repository.complete_scan_run(project_id, scan_id, scan_generation,
                                       failed_metrics, current_unix_time(),
                                       error.what());
        }
      }
...
bool GodotProjectContext::cancel_scan(int64_t scan_id) {
  ...
  active_scan_state_->cancellation_requested.store(true,
...
Dictionary GodotProjectContext::get_scan_status(int64_t scan_id) const {
  ...
  try {
    ...
    const std::string status =
        repository.get_scan_status(current_project_id_, scan_id);
    if (status.empty()) {
      return Dictionary();
    }
    ...
    result["metrics"] = metrics_to_dictionary(
        repository.get_scan_metrics(current_project_id_, scan_id));
  } catch (...) {
    return Dictionary();
  }
...
Dictionary GodotProjectContext::get_scan_metrics(int64_t scan_id) const {
  ...
  try {
    ...
    gotool::project_scanner::ScanRepository repository(*database_);
    Dictionary metrics = metrics_to_dictionary(
        repository.get_scan_metrics(current_project_id_, scan_id));
    metrics["scan_id"] = scan_id;
    metrics["scan_run_id"] = scan_id;
    metrics["status"] =
        from_utf8(repository.get_scan_status(current_project_id_, scan_id));
    return metrics;
  } catch (...) {
    return Dictionary();
  }
...
Dictionary
GodotProjectContext::scan_project_inventory_fast(const Dictionary &options) {
  last_error_ = "";
  ...
  try {
    ...
    const bool include_custom_classes =
        static_cast<bool>(options.get("include_custom_classes", true));
    const bool include_script_dependencies = static_cast<bool>(
        options.get("include_script_dependencies", include_custom_classes));
    const bool materialize_files =
        static_cast<bool>(options.get("materialize_files", true));
    const bool materialize_custom_classes = static_cast<bool>(
        options.get("materialize_custom_classes", include_custom_classes));
    const bool include_autoloads =
        static_cast<bool>(options.get("include_autoloads", true));
    const bool force_rescan =
        static_cast<bool>(options.get("force_rescan", false));
    const bool load_existing_snapshot =
        static_cast<bool>(options.get("load_existing_snapshot", true));
    const bool use_dirty_path_filter =
        static_cast<bool>(options.get("use_dirty_path_filter", false));
    const bool enable_parallel_traversal =
        static_cast<bool>(options.get("enable_parallel_traversal", false));
    const int64_t max_parallel_workers =
        static_cast<int64_t>(options.get("max_parallel_workers", 0));
    const int64_t max_results =
        static_cast<int64_t>(options.get("max_results", 0));
    ...
    scan_options.load_existing_snapshot = load_existing_snapshot;
    scan_options.use_dirty_path_filter =
        use_dirty_path_filter && !dirty_paths.empty() && !force_rescan;
    scan_options.enable_parallel_traversal = enable_parallel_traversal;
    scan_options.max_parallel_workers = max_parallel_workers;
    ...
    if (materialize_files) {
      for (const gotool::project_scanner::EntryRecord &record :
           native_result.records) {
        const std::string project_path = native_result.arena.string_at(
            record.path_offset, record.path_length);
        const std::string file_name = native_result.arena.string_at(
            record.name_offset, record.name_length);
        const std::string extension = native_result.arena.string_at(
            record.extension_offset, record.extension_length);
        const std::filesystem::path absolute_path =
            (project_root / std::filesystem::u8path(project_path))
                .lexically_normal();
        ...
        row["type"] =
            record.entry_kind == gotool::project_scanner::EntryKind::Directory
                ? "folder"
                : "file";
        row["project_relative_path"] = from_utf8(project_path);
        row["absolute_path"] =
            from_utf8(to_utf8_from_u8(absolute_path.generic_u8string()));
        row["file_name"] = from_utf8(file_name);
        row["extension"] = from_utf8(extension);
        row["file_type"] =
            from_utf8(gotool::project_scanner::to_string(record.file_type_id));
        row["godot_type"] = from_utf8(
            gotool::project_scanner::to_string(record.godot_type_hint));
        ...
        row["dirty_state"] =
            from_utf8(gotool::project_scanner::to_string(record.dirty_state));
        row["dirty_reason"] =
            from_utf8(gotool::project_scanner::to_string(record.dirty_reason));
        files.append(row);
        ++materialized_rows;
      }
    }
    ...
    if (include_custom_classes) {
      for (const gotool::project_scanner::ParsedScriptRecord &parsed :
           native_result.parsed_scripts) {
        if (parsed.parse_result.status !=
                gotool::project_scanner::ParseStatus::ParsedClass ||
        ...
        row["script_project_relative_path"] =
            from_utf8(parsed.project_relative_path);
        row["language"] = from_utf8(
            gotool::project_scanner::to_string(parsed.parse_result.language));
        row["base_type"] = from_utf8(direct_base);
        row["direct_base_type"] = from_utf8(direct_base);
        row["is_resource_type"] =
            gotool::project_scanner::is_builtin_resource_type_hint(direct_base);
        row["is_node_type"] =
            gotool::project_scanner::is_builtin_node_type_hint(direct_base);
        row["parser_version"] = gotool::project_scanner::PARSER_VERSION;
        row["parse_status"] = from_utf8(
            gotool::project_scanner::to_string(parsed.parse_result.status));
    ...
    if (include_autoloads) {
      const std::vector<ParsedAutoload> parsed_autoloads =
          parse_project_autoloads(project_root);
      for (const ParsedAutoload &autoload : parsed_autoloads) {
        ...
        row["target_project_relative_path"] =
            from_utf8(gotool::project_scanner::normalize_project_path(
                autoload.target_path));
    ...
    inventory["entry_record_count"] =
        static_cast<int64_t>(native_result.records.size());
    ...
    inventory["scan_summary"] =
        scan_summary_to_dictionary(native_result.summary);
    last_error_ = "";
    return inventory;
  } catch (const std::exception &error) {
    last_error_ = error.what();
...
Dictionary
GodotProjectContext::scan_current_project_fast(const Dictionary &options) {
  return scan_project_inventory_fast(options);
}

Dictionary
GodotProjectContext::benchmark_native_scan(const Dictionary &options) {
  last_error_ = "";
  ...
  try {
    ...
    const bool force_rescan =
        static_cast<bool>(options.get("force_rescan", false));
    const bool load_existing_snapshot =
        static_cast<bool>(options.get("load_existing_snapshot", false));
    ...
    scan_options.collect_custom_classes = include_custom_classes;
    scan_options.collect_script_dependencies = include_script_dependencies;
    scan_options.load_existing_snapshot =
        load_existing_snapshot && include_database_snapshot;
    scan_options.use_dirty_path_filter =
        use_dirty_path_filter && !dirty_paths.empty() && !force_rescan;
    scan_options.enable_parallel_traversal = enable_parallel_traversal;
    scan_options.max_parallel_workers = max_parallel_workers;
    ...
    result["entry_record_count"] =
        static_cast<int64_t>(native_result.records.size());
    result["parsed_script_count"] =
        static_cast<int64_t>(native_result.parsed_scripts.size());
...
int64_t GodotProjectContext::get_file_count(const Dictionary &filter) const {
  ...
  try {
    ...
    gotool::project_scanner::ScanRepository repository(*database_);
    return repository.count_files(current_project_id_,
...
Array GodotProjectContext::get_files_page(int64_t offset, int64_t limit,
                                          const String &sort,
  ...
  try {
    ...
    const std::vector<gotool::project_scanner::FileRow> page =
        repository.list_files(current_project_id_,
                              file_query_from_dictionary(filter), offset, limit,
                              to_utf8(sort));
...
Array GodotProjectContext::get_directory_children(
    int64_t directory_id, int64_t offset, int64_t limit, const String &sort,
    const Dictionary &filter) const {
...
int64_t
GodotProjectContext::get_custom_class_count(const Dictionary &filter) const {
  sync_last_scan_results_from_active_state();
  ...
  try {
    ...
    gotool::project_scanner::ScanRepository repository(*database_);
    return repository.count_custom_classes(
        current_project_id_, custom_class_query_from_dictionary(filter));
  } catch (...) {
    return 0;
  }
}
...
Array GodotProjectContext::get_custom_classes_page(
    int64_t offset, int64_t limit, const String &sort,
    const Dictionary &filter) const {
  Array rows;
  ...
  try {
    ...
    const std::vector<gotool::project_scanner::CustomClassRow> page =
        repository.list_custom_classes(
            current_project_id_, custom_class_query_from_dictionary(filter),
            offset, limit, to_utf8(sort));
...
Array GodotProjectContext::list_dependencies_for_script(
  ...
  try {
    ...
    gotool::project_scanner::ScanRepository repository(*database_);
    const std::vector<gotool::project_scanner::ScriptDependencyRow>
        dependencies = repository.list_dependencies_for_script(
            current_project_id_, script_file_id);

    for (const gotool::project_scanner::ScriptDependencyRow &dependency :
         dependencies) {
...
Array GodotProjectContext::list_dependents_of_file(
  ...
  try {
    ...
    gotool::project_scanner::ScanRepository repository(*database_);
    const std::vector<gotool::project_scanner::ScriptDependencyRow>
        dependencies = repository.list_dependents_of_file(current_project_id_,
                                                          target_file_id);
...
Array GodotProjectContext::list_dependents_of_class(
  ...
  try {
    ...
    gotool::project_scanner::ScanRepository repository(*database_);
    const std::vector<gotool::project_scanner::ScriptDependencyRow>
        dependencies = repository.list_dependents_of_class(current_project_id_,
                                                           class_name_utf8);
...
Array GodotProjectContext::list_symbols_for_script(
    int64_t script_file_id, const Dictionary &filter) const {
  Array rows;
  ...
  try {
    ...
    const std::vector<gotool::project_scanner::ScriptSymbolRow> symbols =
        repository.list_symbols_for_script(
            current_project_id_, script_file_id,
...
Array GodotProjectContext::list_functions_for_script(
    int64_t script_file_id) const {
  Array rows;
  ...
  try {
    ...
    gotool::project_scanner::ScanRepository repository(*database_);
    const std::vector<gotool::project_scanner::ScriptSymbolRow> symbols =
        repository.list_functions_for_script(current_project_id_,
                                             script_file_id);
...
Array GodotProjectContext::list_properties_for_script(
    int64_t script_file_id) const {
  Array rows;
  ...
  try {
    ...
    gotool::project_scanner::ScanRepository repository(*database_);
    const std::vector<gotool::project_scanner::ScriptSymbolRow> symbols =
        repository.list_properties_for_script(current_project_id_,
                                              script_file_id);
...
Array GodotProjectContext::list_parameters_for_function(
  ...
  try {
    ...
    gotool::project_scanner::ScanRepository repository(*database_);
    const std::vector<gotool::project_scanner::ScriptSymbolRow> symbols =
        repository.list_parameters_for_function(current_project_id_,
                                                function_symbol_id);
...
Array GodotProjectContext::list_doc_comment_gaps(
    const Dictionary &filter) const {
  Array rows;
  ...
  try {
    ...
    gotool::project_scanner::ScanRepository repository(*database_);
    const std::vector<gotool::project_scanner::ScriptSymbolRow> symbols =
        repository.list_doc_comment_gaps(
            current_project_id_,
...
Array GodotProjectContext::list_references_for_script(
    int64_t script_file_id) const {
  Array rows;
  ...
  try {
    ...
    const std::vector<gotool::project_scanner::ScriptReferenceRow> references =
        repository.list_references_for_script(current_project_id_,
                                              script_file_id);

    for (const gotool::project_scanner::ScriptReferenceRow &reference :
         references) {
...
Array GodotProjectContext::list_references_from_symbol(
...
Array GodotProjectContext::list_unresolved_references(
    const Dictionary &filter) const {
  Array rows;
  ...
  try {
    ...
    gotool::project_scanner::ScanRepository repository(*database_);
    const std::vector<gotool::project_scanner::ScriptReferenceRow> references =
        repository.list_unresolved_references(
            current_project_id_,
            reference_query_filter_from_dictionary(filter));
...
Array GodotProjectContext::list_dynamic_references(
    const Dictionary &filter) const {
  Array rows;
  ...
  try {
    ...
    gotool::project_scanner::ScanRepository repository(*database_);
    const std::vector<gotool::project_scanner::ScriptReferenceRow> references =
        repository.list_dynamic_references(
            current_project_id_,
            reference_query_filter_from_dictionary(filter));
...
Array GodotProjectContext::list_scene_script_attachments(
    const Dictionary &filter) const {
  Array rows;
  ...
  try {
    ...
    gotool::project_scanner::ScanRepository repository(*database_);
    const std::vector<gotool::project_scanner::SceneScriptAttachmentRow>
        attachments = repository.list_scene_script_attachments(
            current_project_id_,
            scene_attachment_filter_from_dictionary(filter));

    for (const gotool::project_scanner::SceneScriptAttachmentRow &attachment :
         attachments) {
...
Array GodotProjectContext::list_scenes_using_script(
    int64_t script_file_id) const {
  Array rows;
  ...
  try {
    ...
    gotool::project_scanner::ScanRepository repository(*database_);
    const std::vector<gotool::project_scanner::SceneScriptAttachmentRow>
        attachments = repository.list_scenes_using_script(current_project_id_,
                                                          script_file_id);
...
Array GodotProjectContext::list_scripts_attached_to_scene(
  ...
  try {
    ...
    gotool::project_scanner::ScanRepository repository(*database_);
    const std::vector<gotool::project_scanner::SceneScriptAttachmentRow>
        attachments = repository.list_scripts_attached_to_scene(
            current_project_id_, scene_file_id);
...
Dictionary GodotProjectContext::get_script_intelligence_summary(
    int64_t script_file_id) const {
  sync_last_scan_results_from_active_state();
  ...
  try {
    ...
    const gotool::project_scanner::ScriptIntelligenceSummaryRow summary =
        repository.get_script_intelligence_summary(current_project_id_,
                                                   script_file_id);
...
Array GodotProjectContext::get_dependency_graph_slice(
    int64_t root_script_file_id, int64_t depth) const {
  Array rows;

  sync_last_scan_results_from_active_state();

  if (database_ == nullptr || current_project_id_ <= 0 ||
      root_script_file_id <= 0) {
    return rows;
  }
  ...
  try {
    ...
    gotool::project_scanner::ScanRepository repository(*database_);
    const std::vector<gotool::project_scanner::ScriptDependencyRow>
        dependencies = repository.get_dependency_graph_slice(
            current_project_id_, root_script_file_id, depth);
...
Dictionary GodotProjectContext::export_full_inventory_for_debug() const {
  ...
  for (int64_t offset = 0; offset < class_count; offset += 500) {
    const Array page =
        get_custom_classes_page(offset, 500, "class_name", Dictionary());
    for (int64_t i = 0; i < page.size(); ++i) {
...
Array GodotProjectContext::list_projects() const {
  ...
  try {
    ...
    gotool::database::ProjectRegistryRepository registry(*database_);
    const std::vector<gotool::database::ProjectListItem> items =
...
Dictionary GodotProjectContext::get_project_summary(int64_t project_id) const {
  ...
  try {
    ...
    summary["latest_scan_status"] =
        from_utf8(project_summary->latest_scan_status);
