...
[[noreturn]] void throw_sqlite_error(sqlite3 *db, const std::string &context,
  ...
  const std::string message =
      db != nullptr ? sqlite3_errmsg(db) : "sqlite database handle was null";

  throw std::runtime_error(context + " (code " + std::to_string(code) +
                           "): " + message);
}
...
Statement::Statement(sqlite3 *db, const std::string &sql) : db_(db) {
  if (db_ == nullptr) {
    throw std::runtime_error(
        "Cannot prepare statement: sqlite database handle was null.");
  }
...
void Statement::step_done() {
  if (step() != StepResult::Done) {
    throw std::runtime_error("Expected sqlite statement to finish with "
                             "SQLITE_DONE but received a row.");
  }
}
...
Database::Database(const std::string &database_path) {
  ...
  const int result =
      sqlite3_open_v2(database_path.c_str(), &db_, flags, nullptr);
...
void Database::exec(const std::string &sql) {
  if (db_ == nullptr) {
    throw std::runtime_error(
        "Cannot execute sqlite statement: database handle was null.");
  }
  ...
  if (result != SQLITE_OK) {
    const std::string message =
        error_message != nullptr ? error_message : sqlite3_errmsg(db_);
...
Statement Database::prepare(const std::string &sql) {
  if (db_ == nullptr) {
    throw std::runtime_error(
        "Cannot prepare sqlite statement: database handle was null.");
  }
...
void Database::commit_transaction() { exec("COMMIT;"); }
