...
std::optional<ExistingProjectKey> find_project_by_uid(Database &database, const std::string &project_uid) {
...
std::optional<ExistingProjectKey> find_project_by_canonical_root(
    Database &database,
    const std::string &root_canonical_path
) {
    Statement statement = database.prepare(
        "SELECT id, project_uid FROM projects WHERE root_canonical_path = ?1 LIMIT 1;"
    );
...
ProjectRegistryRepository::ProjectRegistryRepository(Database &database) :
    ...
    if (database_ == nullptr) {
        throw std::runtime_error("ProjectRegistryRepository requires a valid Database instance.");
    }
}

RegisteredProject ProjectRegistryRepository::register_project(const ProjectRegistrationInput &input) {
    if (input.project_uid.empty()) {
        throw std::runtime_error("Cannot register project: project_uid was empty.");
    }

    const std::string root_absolute_path = normalize_path_string(input.root_absolute_path);
    const std::string root_canonical_path = normalize_path_string(input.root_canonical_path);
    const std::string project_file_absolute_path = normalize_path_string(input.project_file_absolute_path);

    if (root_canonical_path.empty()) {
        throw std::runtime_error("Cannot register project: canonical root path was empty.");
    }

    if (project_file_absolute_path.empty()) {
        throw std::runtime_error("Cannot register project: project.godot absolute path was empty.");
    }

    const int64_t observed_at_unix =
        input.observed_at_unix > 0
    ...
    if (existing_by_uid.has_value() &&
        existing_by_canonical_root.has_value() &&
        existing_by_uid->project_id != existing_by_canonical_root->project_id) {
        throw std::runtime_error(
            "Project identity conflict: project_uid '" + input.project_uid +
            "' maps to a different row than canonical root '" + root_canonical_path + "'."
        );
    }

    if (!existing_by_uid.has_value() && existing_by_canonical_root.has_value()) {
        throw std::runtime_error(
            "Project identity conflict: canonical root '" + root_canonical_path +
            "' is already associated with project_uid '" + existing_by_canonical_root->project_uid +
...
std::optional<ProjectSummary> ProjectRegistryRepository::get_project_summary(int64_t project_id) const {
    ...
    if (count_custom_classes_statement.step() == Statement::StepResult::Row) {
        summary.custom_classes_count = count_custom_classes_statement.column_int64(0);
    }
...
std::string ProjectRegistryRepository::normalize_path_string(const std::filesystem::path &path) {
    std::filesystem::path normalized = path.lexically_normal();
    return normalized.make_preferred().string();
}

ProjectListItem ProjectRegistryRepository::read_project_row(Statement &statement) const {
